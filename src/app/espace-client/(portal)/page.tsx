...
export default async function DashboardPage() {
  ...
  return (
    <div className="space-y-14">
      <section>
        <dl className="grid grid-cols-2 gap-px border border-parch-200/12 bg-parch-200/10 lg:grid-cols-4">
          {stats.map((stat) => (
            <div key={stat.label} className="bg-wine-900 px-5 py-8 text-center">
              <dt className="font-display text-3xl font-light text-gold-400">
                {stat.value}
              </dt>
              <dd className="mt-2 text-[0.72rem] leading-snug text-parch-400">
                {stat.label}
              </dd>
            </div>
          ))}
        ...
        <div className="flex items-baseline justify-between gap-6">
          ...
          {upcoming.length > 0 ? (
            <Link
              ...
              className="text-[0.66rem] tracking-[0.18em] text-gold-400 uppercase transition-colors hover:text-gold-300"
            >
              Tout voir →
            </Link>
          ) : null}
