...
const fieldBase =
  "w-full border border-vine-900/22 bg-sand-50 px-4 py-3.5 text-[0.95rem] text-vine-900 transition-colors placeholder:text-vine-400 focus:border-tuile-600 focus:outline-none";
...
export function Field({
  ...
  return (
    <div className={className}>
      ...
      {errors?.length ? (
        <p className="mt-2 text-[0.75rem] text-tuile-600">{errors[0]}</p>
      ) : null}
    </div>
  );
}
...
export function SubmitButton({
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className={`inline-flex items-center justify-center gap-2 bg-tuile-600 px-8 py-4 text-[0.72rem] font-medium tracking-[0.2em] text-sand-50 uppercase transition-all duration-300 hover:bg-tuile-700 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-tuile-600 disabled:cursor-not-allowed disabled:opacity-60 ${className}`}
    >
  ...
  return (
    <div
      ...
      className={`border px-5 py-4 text-[0.85rem] leading-relaxed ${
        tone === "error"
          ? "border-tuile-600/45 bg-tuile-100 text-tuile-700"
